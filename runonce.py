...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("historical_metrics.log"),
    ...
    try:
        with connect_to_db() as conn:
            with conn.cursor() as cur:
                ...
                logger.info(f"Fetching data for device {device_id} from {start_date}")
...
def process_vibration_data(data, sensor_id, positive_threshold, negative_threshold):
    ...
    for date, day_data in data.groupby('date'):
        ...
        day_data['time_diff'] = (day_data['next_time'] - day_data['time']).dt.total_seconds()
...
def insert_machine_metrics(device_id, metric_type, date, metric_data):
    ...
    try:
        with connect_to_db() as conn:
            with conn.cursor() as cur:
                ...
                if existing:
                    ...
                    logger.info(f"Entry already exists for device {device_id}, date {date}, skipping")
                else:
                    # Insert new entry
                    query = """
        ...
        logger.error(f"Error inserting machine metrics for device {device_id}, date {date}: {e}")
